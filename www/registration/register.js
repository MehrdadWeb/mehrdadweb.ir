...
function CheckUsername(input) {
    if ($(input).val().trim() == "") {
        $("#username_alert").html("جیگرنام کاربری را وارد کنید.");
        ...
        $.ajax({
            ...
            success: function(res) {
                if (res.trim() == 'structure_username_error') {
                    $("#username_alert").html("نام کاربری باید فقط شامل تمامی حروف انگلیسی و اعداد باشدجیگر.");
                    $("#username_check").css("background-image", "url('/front/img/URLShortener/input-cross.png')");
...
function RegisterMe() {
    ...
    if ($('#username_check').attr("data-status") == "0"){alert('username_check error');}
    if ($('#password_check').attr("data-status") == "0"){alert('password_check error');}
    if ($('#email_check').attr("data-status") == "0"){alert('email_check error');}
